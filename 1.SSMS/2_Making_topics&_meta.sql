...
DECLARE @ForumId INT, @TopicId INT, @LastReplyId INT, @LastPostDate DATETIME, 
        ...
        @StatusId INT, @NewTopicId INT = 1;  -- Start from 1 for the new Topic ID

-- Cursor to iterate over approved topics in activeforums_Topics
DECLARE TopicCursor CURSOR FOR
SELECT T.TopicId, FT.ForumId, FT.LastReplyId, FT.LastPostDate, T.StatusID
...
FETCH NEXT FROM TopicCursor INTO @TopicId, @ForumId, @LastReplyId, @LastPostDate, @StatusId;

-- Loop through each approved topic
WHILE @@FETCH_STATUS = 0
BEGIN
    -- Retrieve the ContentId from the activeforums_Topics table for the current TopicId
    DECLARE @ContentId INT, @ReplyCount INT, @ViewCount INT;
    SELECT @ContentId = ContentId, @ReplyCount= ReplyCount, @ViewCount= ViewCount FROM [SimorghPortalZero972].[dbo].[activeforums_Topics] WHERE TopicId = @TopicId;
    ...
    VALUES (
        ...
        @ReplyCount,                              -- posts default to 0
        0,                              -- votes default to 0
        0,                              -- answers default to 0
        @ViewCount,                              -- views default to 0
        NULL,                           -- meta_key
        NULL,                           -- meta_desc
        CASE WHEN @StatusId = 0 THEN 1 ELSE 0 END,  -- type (1 if StatusId is 0, otherwise 0)
        CASE WHEN @StatusId = 3 THEN 1 ELSE 0 END,                         -- solved default to 0
